...
Moog :: Moog( void )
{
	...
	const char *mandplukFilePath = [[[NSBundle mainBundle] pathForResource:@"mandpluk" ofType:@"raw"] UTF8String];
	const char *impuls20FilePath = [[[NSBundle mainBundle] pathForResource:@"impuls20" ofType:@"raw"] UTF8String];	
	const char *sinewaveFilePath = [[[NSBundle mainBundle] pathForResource:@"sinewave" ofType:@"raw"] UTF8String];
	
  // Concatenate the STK rawwave path to the rawwave file
  attacks_.push_back( new FileWvIn(mandplukFilePath, true ) );
  loops_.push_back ( new FileLoop( impuls20FilePath, true ) );
  loops_.push_back ( new FileLoop(sinewaveFilePath, true ) ); // vibrato
