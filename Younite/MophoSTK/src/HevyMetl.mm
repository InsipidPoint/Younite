...
HevyMetl :: HevyMetl( void )
  : FM()
{
	const char *sineFilePath = [[[NSBundle mainBundle] pathForResource:@"sinewave" ofType:@"raw"] UTF8String];
	const char *fwavblnkFilePath = [[[NSBundle mainBundle] pathForResource:@"fwavblnk" ofType:@"raw"] UTF8String];	

	
  // Concatenate the STK rawwave path to the rawwave files
  for ( unsigned int i=0; i<3; i++ )
    waves_[i] = new FileLoop( sineFilePath, true );
  waves_[3] = new FileLoop( fwavblnkFilePath, true );
