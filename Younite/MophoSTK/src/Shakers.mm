...
int Shakers :: setFreqAndReson( int which, StkFloat freq, StkFloat reson )
{
  if ( which < MAX_FREQS )	{
    ...
    coeffs_[which][0] = -reson * 2.0 * cos(freq * STK_TWO_PI / Stk::sampleRate());
    return 1;
  }
  else return 0;
}
      ...
      if (float_random(1024.0) < nObjects_) {
        ...
        for (i=0;i<nFreqs_;i++) {
          if (freqalloc_[i])	{
            ...
            coeffs_[i][0] = -resons_[i] * 2.0 * cos(temp_rand * STK_TWO_PI / Stk::sampleRate());
          }
        }
    	}
...
void Shakers :: controlChange( int number, StkFloat value )
{
  ...
  if (number == __SK_Breath_) { // 2 ... energy
    ...
    for (i=0; i<nFreqs_; i++)	{
      ...
      coeffs_[i][0] = -resons_[i] * 2.0 * cos(temp * STK_TWO_PI / Stk::sampleRate());
...
StkFloat Shakers :: wuter_tick( void ) {
  ...
  if (gains_[0] >  0.001) {
    ...
    coeffs_[0][0] = -resons_[0] * 2.0 * 
      cos(center_freqs_[0] * STK_TWO_PI / Stk::sampleRate());
  }
  gains_[1] *= resons_[1];
  if (gains_[1] > 0.001) {
    center_freqs_[1] *= WUTR_FREQ_SWEEP;
    coeffs_[1][0] = -resons_[1] * 2.0 * 
      cos(center_freqs_[1] * STK_TWO_PI / Stk::sampleRate());
  }
  gains_[2] *= resons_[2];
  if (gains_[2] > 0.001) {
    center_freqs_[2] *= WUTR_FREQ_SWEEP;
    coeffs_[2][0] = -resons_[2] * 2.0 * 
      cos(center_freqs_[2] * STK_TWO_PI / Stk::sampleRate());
  }
